...
def multipart_constructor(image: str = "" or bytes):
    ...
    return MultipartEncoder(fields={"file": get_file(image)})


def multipart_constructor_with_two_images(
    source_image: str = "" or bytes, target_image: str = "" or bytes
):
    return MultipartEncoder(
        fields={
            "source_image": get_file(source_image),
            "target_image": get_file(target_image),
        }
    )
